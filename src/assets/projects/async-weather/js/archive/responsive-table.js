(function () {
  ...
  var resSel = document.getElementById("responsive-selector");
  console.log(resSel);
  var responsive = resSel.options[resSel.selectedIndex].text;
  console.log(responsive);
  
  /*  
    Working varilable:  
    City:   Name	ID	COD	Message	Longitutde	Latitude	Country	Population	SYS POP	CNT
    Day:    Day	Min	Max	Night	Evening	Morning	Pressure	Humidity	ID	Icon	Expected Weather	Weather Description	Speed	Direction	Clouds	Rain
   */
  
  /* So, on page load or on the selector change:
     1.  Change the show array of varilables
     2.  Run the fuctions (city and day) again so they will redraw the tables.
        2a.  The tables might need to be modified to use effective space, 
              issolate the enviroment in the specific function and template it.
     3.  Research the update at openweather and set to that value, it's pining too often
     
  */
  
  /*  Three conditions;  page width changes size, option selects size, media selects size  
  
  /* impliment:  
  function load(){
     if(window.outerheight){
       w = windows.outerwidth,
       h = window.outerheight;
     } else {                             //For IE8 and below
     w = docuement.body.client.width,
     h = document.body.client.height;
     }
  }
   */ 
  ...
  if (responsive =="web layout") {
    domContent.innerHTML += "Web Selected";
    console.log('web trigger');
  } else if (responsive == "mobile layout") {
    domContent.innerHTML += "Mobile Selected";
    console.log('mobile trigger');
  } else {
    domContent.innerHTML += "How did you do that?";
