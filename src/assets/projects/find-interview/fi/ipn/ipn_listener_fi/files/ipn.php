...
$N_Valid = date('d-m-Y', strtotime($C_Valid. ' + ' . $NumOfDays . ' days'));
	...
	$fp = fopen('data.txt', 'a');
	fwrite($fp, "-" . $payment_amount . "||||||");
	fclose($fp);

$update_query = "UPDATE `users` SET `valid_through` = '$N_Valid' WHERE `users`.`id` =$UserID;" ; 
// $update_query = "INSERT INTO `users` (`id`, `username`, `password`, `credits`, `expire`) VALUES (NULL, '$payer_email', '$randpwd_insert', '$credits', '07/07/2000') ON DUPLICATE KEY UPDATE credits = credits + $credits , password = '$randpwd' ;" ; 
$do_update_query = mysql_query($update_query); 

	mail($payer_email,'Account Into',"Thank you for buying/recharging your credit.") ; 
    	mail($ipn_email, 'Verified IPN', $listener->getPostVars());
	
	$fp = fopen('data.txt', 'a');
	fwrite($fp, "-" . $listener->getPostVars() . "||||||");
	fclose($fp);
    ...
    invalid IPN.
    */
    mail($ipn_email, 'Invalid IPN', $listener->getPostVars());
