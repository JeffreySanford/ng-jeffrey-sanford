...
$GetPartner1 = $user->GetUserInfo("id") ; 
$GetPartner2 = $functions->SecureInput($_GET['with']) ; 
$cmd = $functions->SecureInput($_GET['cmd']) ; 

if($cmd=="reject"){
	$jobs->RejectRoom($GetPartner1,$GetPartner2) ; 
	exit() ; 
}

$room_key = $jobs->StartRoom($GetPartner1,$GetPartner2) ; 
$my_name = $user->GetUserInfo("id") . "_" . $user->GetUserInfo("first_name") . " " . $user->GetUserInfo("last_name") ; 
		...
		<script>
			$(document).ready(function() {
				$("#webcam").scriptcam({ 
					...
					setVolume:setVolume,
					timeLeft:timeLeft,
					loginName:'<?php echo $my_name ; ?>',
					chatRoom:'<?php echo $room_key ; ?>'
				});
				setVolume(0);
