...
$user->CheckAuth(True) ; 
$vtype = $_GET['vtype'] ; 
if($vtype=="apply_for_job"){
	$vtype = $vtype . "&id=" . $functions->SecureInput($_GET['id']) ; 
}elseif($vtype=="post_job"){
	$skip_link = "<a href='../dashboard/rec/post_job.php'>Skip Video</a>" ; 
}
...
<html>
	<head>
		...
		<script>
			...
			function fileReady(fileName) {
				$('#recorder').hide();
				window.location.href = "process_video.php?vtype=<?php echo $vtype ; ?>&vlink=" + fileName;
	...
	<body style="background:#C6C6C6;">
		<center>
		<div class="panel panel-primary" style="width:30%;"><div class="panel-body">
		<div id="message"></div>
		<div id="recorder">
			<div id="webcam">
			</div>
			...
			<input type="text" name="azaz" id="azaz" />
		</div>
		<div id="mediaplayer" style="display:none;"></div>
		</div><div class="panel-footer" style="color:navy;">Please record your video. Then you'll be redirected to continue ... <?php echo $skip_link ; ?></div></div>
