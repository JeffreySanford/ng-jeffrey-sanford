...
class Jobs{
	...
	function RemoveAV($id){
		$remove_query = mysql_query("DELETE FROM `apply_for_job` WHERE `id` = $id ;"); 
	}
	...
	function GetAV($job_id = NULL,$user_id = NULL,$video_id = NULL,$limit = NULL){
		...
		while ($fetch_info= mysql_fetch_array($DoGetOrdersQuery)) {
					...
					$user_id =  $fetch_info['user_id'] ; 
					$apply_video =  $fetch_info['apply_video'] ;  
					$return = $return . '<tr><th>' . $id . '</th><th>' . $job_id . '</th><th>' . $user_id . '</th><th><a href="../user_files/Videos/Jobs/' . $apply_video . '"> <button id="download" name="download" class="btn btn-success">Download</button></a></th><th><a href="./process.php?cmd=Remove&remove_type=av&id=' . $id . '"> <button id="remove" name="remove" class="btn btn-danger">Remove</button></a></th></tr>' ; 
		}
